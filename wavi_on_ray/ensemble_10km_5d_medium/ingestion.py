...
def build_hdf_data(datasets_folder: Union[str, pathlib.Path]):
    """Extracts input and output data frames from the ensemble_10km_5D_medium.nc dataset.

    Args:
        datasets_folder (Union[str, pathlib.Path]): Path to the DVC datasets folder.
    """
    datasets = pathlib.Path(datasets_folder)
    dataset = xr.open_dataset(datasets / "ensemble_10km_5D_medium.nc")

    inputs = pd.DataFrame(
        {
            "melt_in_partial_cells": dataset["Inputs"][0],
            "gammaT": dataset["Inputs"][1],
            "melt_exp": dataset["Inputs"][2],
            "glen_n": dataset["Inputs"][3],
            "weertman_m": dataset["Inputs"][4],
        }
    )
    outputs = pd.DataFrame({"SLC": dataset["SLC"][0]})
    inputs_outputs = pd.concat([inputs, outputs], axis=1)

    prepared_folder = datasets / "prepared" / "ensemble_10km_5D_medium"
