...
def build_hdf_data(datasets_folder: Union[str, pathlib.Path]):
    """Extracts input and output data frames from the ensemble_10km_basin_specific.nc dataset.

    Args:
        datasets_folder (Union[str, pathlib.Path]): Path to the DVC datasets folder.
    """
    datasets = pathlib.Path(datasets_folder)
    dataset = xr.open_dataset(datasets / "ensemble_10km_basin_specific.nc")

    inputs = pd.DataFrame(
        {
            "melt_rate_basin_21": dataset["Inputs"][0],
            "melt_rate_basin_22": dataset["Inputs"][1],
            "melt_in_partial_cells": dataset["Inputs"][2],
        }
    )
    outputs = pd.DataFrame({"SLC": dataset["SLC"][0]})
    inputs_outputs = pd.concat([inputs, outputs], axis=1)

    prepared_folder = datasets / "prepared" / "ensemble_10km_basin_specific"
...
def ingest(datasets_folder):
    """Ingest ensemble_10km_basin_specific data for emulator training."""
